...
export default function ShopByGram() {
    ...
    <section className="py-6 xs:py-9 md:py-16 bg-white">
      <div className="container mx-auto p-4">
        <div className="text-center mb-10">
          <h2 className="text-2xl xs:text-3xl md:text-4xl font-semibold">Shop by gram</h2>
        ...
        >
          {collections.map((item, index) => (
            <SwiperSlide key={index}>
              <Link
                href={item.link}
                className="relative block overflow-hidden rounded-xl aspect-1/1"
              >
                <img
                  src={item.image}
                  alt={item.title}
                ...
                />
                <div className="absolute top-2 right-2 bg-black p-3 rounded-full">
                    <ShoppingBag className="w-4 h-4 relative text-white" />
                </div>
              </Link>
            </SwiperSlide>
          ))}
